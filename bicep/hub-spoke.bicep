// samples/solutions/azure-hub-spoke at main · mspnp/samples https://github.com/mspnp/samples/tree/main/solutions/azure-hub-spoke
...
@description('Hub VNet ネットワークアドレス')
param addressPrefix string = '10.0.0.0/22'
@description('Hub VNet Bastionサブネットアドレス')
param bastionSubnetPrefix string = '10.0.1.0/26'
@description('Hub VNet Gatewayサブネットアドレス')
param gatewaySubnetPrefix string = '10.0.2.0/27'
@description('Hub VNet Firewallサブネットアドレス')
param firewallSubnetPrefix string = '10.0.3.0/26'

@description('Spoke VNet-1 ネットワークアドレス')
param spokeAddressPrefix string = '10.100.0.0/22'
@description('Spoke VNet-1 サブネットアドレス')
param spokeSubnetPrefix string = '10.100.0.0/24'
@description('Spoke VNet-1 PrivateLink Endpoint')
param spokePrivateLinkEndpoints string = '10.100.1.0/26'
...
@description('Linux Strage AccountType')
@allowed([
  'StandardSSD_LRS'
  'Standard_LRS'
  'Premium_LRS'
])
param linuxStrageAccountType string = 'StandardSSD_LRS'
@description('Linux Name')
param linuxComputerName string = 'examplevm-linux'
...
@description('Spoke VNet-2 ネットワークアドレス')
param spoke2AddressPrefix string = '10.200.0.0/22'
@description('Spoke VNet-2 サブネットアドレス')
param spoke2SubnetPrefix string = '10.200.0.0/24'
@description('Spoke VNet-2 PrivateLink Endpoint')
param spoke2PrivateLinxEndpoints string = '10.200.1.0/26'
...
@description('Linux Name')
param windowsComputerName string = 'examplevm-windows'
@description('Linux Strage AccountType')
@allowed([
  'StandardSSD_LRS'
  'Standard_LRS'
...
param windowsStrageAccountType string = 'StandardSSD_LRS'
@allowed([
  '2008-R2-SP1'
  '2012-R2-Datacenter'
  '2016-Datacenter'
...
resource nsgBastionSubnet 'Microsoft.Network/networkSecurityGroups@2022-05-01' = {
...
resource vnetHub 'Microsoft.Network/virtualNetworks@2022-01-01' = {
  ...
  resource peerToSpokeOne 'virtualNetworkPeerings@2022-05-01' = {
  ...
  resource peerToSpokeTwo 'virtualNetworkPeerings@2022-05-01' = {
...
resource pipsAzureFirewall 'Microsoft.Network/publicIPAddresses@2022-05-01' = [for i in range(0, numFirewallIpAddressesToAssign): {
...
resource fwPolicy 'Microsoft.Network/firewallPolicies@2022-05-01' = {
  ...
  resource defaultNetworkRuleCollectionGroup 'ruleCollectionGroups@2022-05-01' = {
  ...
  resource defaultApplicationRuleCollectionGroup 'ruleCollectionGroups@2022-05-01' = {
          ...
          rules: deployVirtualMachines ? [
            {
              ...
              sourceAddresses: [
                spoke2SubnetPrefix // The subnet that contains the Windows VMs
              ]
            }
          ] : []
        }
...
resource fwHub 'Microsoft.Network/azureFirewalls@2022-05-01' = {
  name: 'fw-${location}'
  location: location
  zones: [
    '1'
...
resource pipAzureBastion 'Microsoft.Network/publicIPAddresses@2022-05-01' = {
...
resource pipVpnGateway 'Microsoft.Network/publicIPAddresses@2022-05-01' = if (deployVpnGateway) {
...
resource vgwHub 'Microsoft.Network/virtualNetworkGateways@2022-05-01' = if (deployVpnGateway) {
...
resource routeNextHopToFirewall 'Microsoft.Network/routeTables@2022-05-01' = {
...
resource nsgResourcesSubnet 'Microsoft.Network/networkSecurityGroups@2022-05-01' = {
...
resource nsgPrivateLinkEndpointsSubnet 'Microsoft.Network/networkSecurityGroups@2022-05-01' = {
...
resource vnetSpokeOne 'Microsoft.Network/virtualNetworks@2022-05-01' = {
  ...
  properties: {
    ...
    subnets: [
      ...
      }
      {
        ...
        properties: {
          addressPrefix: spokePrivateLinkEndpoints
          networkSecurityGroup: {
  ...
  resource peerToHub 'virtualNetworkPeerings@2022-05-01' = {
...
resource nicVmSpokeOneLinux 'Microsoft.Network/networkInterfaces@2022-05-01' = if (deployVirtualMachines) {
...
resource vmSpokeOneLinux 'Microsoft.Compute/virtualMachines@2022-08-01' = if (deployVirtualMachines) {
  ...
  properties: {
    ...
    storageProfile: {
      osDisk: {
        ...
        managedDisk: {
          storageAccountType: linuxStrageAccountType
        }
        caching: 'ReadOnly'
        diffDiskSettings: {
          option: 'Local'
...
resource vnetSpokeTwo 'Microsoft.Network/virtualNetworks@2022-05-01' = {
  ...
  properties: {
    ...
    subnets: [
      ...
      }
      {
        name: 'snet-privatelinkendpoints'
        properties: {
          addressPrefix: spoke2PrivateLinxEndpoints
          networkSecurityGroup: {
            id: nsgPrivateLinkEndpointsSubnet.id
          }
          privateEndpointNetworkPolicies: 'Enabled'
...
resource nicVmSpokeTwoLinux 'Microsoft.Network/networkInterfaces@2022-05-01' = if (deployVirtualMachines) {
...
resource vmSpokeTwoWindows 'Microsoft.Compute/virtualMachines@2022-08-01' = if (deployVirtualMachines) {
  ...
  properties: {
    ...
    storageProfile: {
      osDisk: {
        ...
        caching: 'ReadWrite'
        managedDisk: {
          storageAccountType: windowsStrageAccountType
        }
        deleteOption: 'Delete'
      }
      imageReference: {
